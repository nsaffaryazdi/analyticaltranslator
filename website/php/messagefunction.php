...
//2. The sentence is received and split into words and assigned to an array.
    ...
    genderconrcondances($numberofwords,$category,$genero,$case,$type,$neworder,$wordorder,$spanish,$word,$cword,$message);	//10. Translate genders to the target language
    ...
    jsonencode($message);		//13. Function to send results
...
function capitalizedsubstantives (&$numberofwords,&$category,&$word,&$message)
{

	    for ($i=0; $i<=$numberofwords; $i++) 
	    {
		  if ($category[$i]=="substantive")
		  {
			    $word[$i]=ucfirst($word[$i]);
			    //$message[0]="The word $word[$i] is a substantive and should be capitalized";
		  }
	    }
}
...
//12.1 Declination concordances
	...
	*/
	...
	//Genitivo neutral Ich gebe deiner kind
     ...
     function genderconrcondances (&$numberofwords,&$category,&$genero,&$case,&$type,&$neworder,&$wordorder,&$spanish,&$word,&$cword,&$message)
{
...
//12.2. Gender concordances
    ...
    if($category[$i] == "artdef" && $category[$imasuno] == "substantive" && ($type[$i]!=$type[$imasuno])) {
	
		$message[0]="Error! Genders are not correct! the article $word[$i] is $type[$i] because the substantive $word[$imasuno] is $type[$imasuno] and in $case[$imasuno] case.";
		
    if(!$cword[$i]){  }else{ $word[$i]=$cword[$i];}
      ...
      /*$message[2]="Please check the following cases and declinations table:      
